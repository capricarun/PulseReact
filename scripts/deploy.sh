...
# Required: BACKEND_IMAGE, FRONTEND_IMAGE (full image references, e.g. <account>.dkr.ecr.<region>.amazonaws.com/pulse-backend:12-a1b2c3d)
# Optional: K8S_NAMESPACE (default: pulse), ROLLOUT_TIMEOUT (default: 300s)
set -euo pipefail

: "${BACKEND_IMAGE:?BACKEND_IMAGE is required}"
: "${FRONTEND_IMAGE:?FRONTEND_IMAGE is required}"
NS="${K8S_NAMESPACE:-pulse}"
TIMEOUT="${ROLLOUT_TIMEOUT:-300s}"

cd "$(dirname "$0")/.."

# The public URL of the ALB (used as the backend's CORS origin)
ALB_HOST="$(kubectl -n "${NS}" get ingress pulse -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null || true)"
...
kubectl -n "${NS}" get deployments,replicasets,pods -l app.kubernetes.io/part-of=pulse -o wide
